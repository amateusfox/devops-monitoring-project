pipeline {
    ...
    environment {
        // Загружаем переменные из файла .env через credentials
        ENV_FILE = credentials('monitoring-env-file')
        
        // Или отдельные переменные (альтернативный способ)
    ...
    stages {
        ...
        stage('Load Environment Variables') {
            steps {
                // Способ 1: Из файла .env (если загружен как secret file)
                script {
                ...
                // Выводим переменные для отладки (без паролей)
                sh '''
        ...
        stage('Prepare Ansible Environment') {
            steps {
                // Копируем SSH ключ из Jenkins credentials
                ...
                // Обновляем inventory.ini с актуальными IP
                sh '''
...
EOF
                ...
                // Устанавливаем Ansible если нет
                sh '''
            ...
            steps {
                ...
                sh """
                    ...
                    for DOMAIN in ${DOMAIN_PROM} ${DOMAIN_GRAFANA}; do
                        ...
                        # Проверяем, что срок действия больше 7 дней
                ...
                // 4. Проверка метрик Node Exporter
                sh """
                    echo "=== Checking Node Exporter Metrics ==="
                    # Проверяем метрики с control-vm
                    METRICS=\$(curl -s -f -L --max-time 30 https://${DOMAIN_PROM}/api/v1/query?query=up{job="node_exporter_control"})
                    echo "Metrics response: \$METRICS"
                    
                    if echo "\$METRICS" | grep -q '"result":\[\]'; then
                        echo "ERROR: No metrics found for node_exporter_control"
                        exit 1
                    else
                    ...
                    curl -s https://${DOMAIN_PROM}/metrics | grep -E 'node_cpu|node_memory|node_filesystem' | head -5
                    echo "Key metrics found"
                """
                
                // 5. Проверка работы Docker контейнеров
                sh """
                    ...
                    ssh -i ~/.ssh/id_ansible_ed25519 -o StrictHostKeyChecking=no devops@${MONITORING_VM_IP} "
                        ...
                        echo 'Running: \$RUNNING, Total: \$((TOTAL-1))'
        ...
        stage('Smoke Tests') {
            steps {
                sh """
                    ...
                    # Проверяем, что Prometheus собирает метрики
                    QUERY='count(up)'
                    RESULT=\$(curl -s "https://${DOMAIN_PROM}/api/v1/query?query=\$QUERY")
                    echo "Query result: \$RESULT"
                    
                    # Проверяем, что Nginx проксирует правильно
    ...
    post {
        success {
            ...
            sh """
                ...
                echo 'Default Grafana credentials: admin / ${GRAFANA_PASS}'
            """
        }
        failure {
